...
def flat_output(T,t,maxderi,eta_0,eta_T):
    """ Extendend Nonlinear System Model
        ...
        eta_0:         Startwert flacher Ausgang am Anfang
        eta_T:         Endwert flacher Ausgang


        Returns
        --------
        eta:       flat output and his derivativs as vector 
    ...
    for derivative in range(0,maxderi):
        ...
        if derivative==0:
            eta[derivative,:] = eta_0 + (eta_T - eta_0) * poly_transition(tau,0,maxderi+1)
        else:
            eta[derivative,:] = (eta_T- eta_0) * poly_transition(tau,derivative,maxderi+1)/T**derivative 
